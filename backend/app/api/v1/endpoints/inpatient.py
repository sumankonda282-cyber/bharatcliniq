...
def admission_timeline(
    ...
    """Merged chronological feed of ALL clinical events — full data for chart rendering."""
    adm = db.query(Admission).filter(
        Admission.id == admission_id,
        Admission.clinic_id == current.clinic_id,
    ).first()
    ...
    # ── Vitals ──────────────────────────────────────────────────────────────────
    for v in db.query(VitalSign).filter(VitalSign.admission_id == admission_id).all():
        parts = []
        if v.temperature is not None: parts.append(f"Temp {v.temperature}°C")
        if v.pulse is not None: parts.append(f"Pulse {v.pulse} bpm")
        if v.bp_systolic is not None: parts.append(f"BP {v.bp_systolic}/{v.bp_diastolic} mmHg")
        if v.spo2 is not None: parts.append(f"SpO2 {v.spo2}%")
        if v.respiration_rate is not None: parts.append(f"RR {v.respiration_rate}/min")
        if v.pain_score is not None: parts.append(f"Pain {v.pain_score}/10")
        if v.weight is not None: parts.append(f"Wt {v.weight} kg")
        events.append({
            "type": "vitals",
            "entry_id": v.id,
            "timestamp": v.recorded_at.isoformat() if v.recorded_at else None,
            "written_by_name": _staff_name(v.recorded_by),
            "data": {
                "temperature": float(v.temperature) if v.temperature is not None else None,
                "pulse": v.pulse,
                "bp_systolic": v.bp_systolic,
            ...
            "rendered_text": " · ".join(parts) if parts else "Vitals recorded",
        })

    # ── Nursing Notes ────────────────────────────────────────────────────────────
    for n in db.query(NursingNote).filter(NursingNote.admission_id == admission_id).all():
        events.append({
            "type": "nursing_note",
            "entry_id": n.id,
            "timestamp": n.written_at.isoformat() if n.written_at else None,
            "written_by_name": _staff_name(n.written_by),
            "data": {
                "note_type": n.note_type,
                "note_text": n.note_text,
                "shift": n.shift,
            ...
            "rendered_text": n.note_text or "",
        })

    # ── Progress Notes (SOAP) ────────────────────────────────────────────────────
    for n in db.query(ProgressNote).filter(ProgressNote.admission_id == admission_id).all():
        parts = []
        if n.subjective: parts.append(f"S: {n.subjective}")
        if n.objective: parts.append(f"O: {n.objective}")
        if n.assessment: parts.append(f"A: {n.assessment}")
        if n.plan: parts.append(f"P: {n.plan}")
        events.append({
            "type": "progress_note",
            "entry_id": n.id,
            "timestamp": n.note_time.isoformat() if n.note_time else None,
            "written_by_name": _staff_name(n.written_by),
            "data": {
                "note_type": n.note_type,
                "subjective": n.subjective,
            ...
            "rendered_text": "\n".join(parts) if parts else f"{n.note_type} note",
        })

    # ── Ward Rounds (SOAP) ───────────────────────────────────────────────────────
    for r in db.query(WardRound).filter(WardRound.admission_id == admission_id).all():
        parts = []
        if r.subjective: parts.append(f"S: {r.subjective}")
        if r.objective: parts.append(f"O: {r.objective}")
        if r.assessment: parts.append(f"A: {r.assessment}")
        if r.plan: parts.append(f"P: {r.plan}")
        ts = datetime.combine(r.round_date, datetime.min.time()) if r.round_date and not r.created_at else r.created_at
        events.append({
            "type": "ward_round",
            "entry_id": r.id,
            "timestamp": ts.isoformat() if ts else None,
            "written_by_name": _staff_name(r.doctor_id),
            "data": {
                "subjective": r.subjective,
                "objective": r.objective,
                "assessment": r.assessment,
            ...
            "rendered_text": "\n".join(parts) if parts else "Ward Round",
        })

    # ── MAR Administrations ──────────────────────────────────────────────────────
    for m in db.query(MedicationAdministration).filter(
        MedicationAdministration.admission_id == admission_id,
        MedicationAdministration.status.in_(["given", "held", "refused", "missed"]),
    ).all():
        ts = m.administered_at or m.scheduled_time
        action = {
            "given": "Administered",
            "held": "Held",
            "refused": "Refused",
        ...
        text = f"{action}: {m.medicine_name}"
        if m.dose: text += f" {m.dose}"
        if m.route: text += f" ({m.route})"
        if m.status == "held" and m.reason_held: text += f" — Reason: {m.reason_held}"
        if m.notes: text += f"\nNote: {m.notes}"
        events.append({
            "type": "medication",
            "entry_id": m.id,
            "timestamp": ts.isoformat() if ts else None,
            "written_by_name": _staff_name(m.administered_by),
            "data": {
                "medicine_name": m.medicine_name,
                "dose": m.dose,
                "route": m.route,
                "status": m.status,
                "reason_held": m.reason_held,
                "notes": m.notes,
            },
            "rendered_text": text,
        })
    ...
    # ── Transfers ────────────────────────────────────────────────────────────────
    for t in db.query(AdmissionTransfer).filter(AdmissionTransfer.admission_id == admission_id).all():
        events.append({
            "type": "transfer",
            "entry_id": t.id,
            "timestamp": t.transferred_at.isoformat() if t.transferred_at else None,
            "written_by_name": _staff_name(t.transferred_by),
            "data": {"reason": t.reason},
            "rendered_text": t.reason or "Patient transferred to new location",
        })

    # ── Documentation Sessions (dividers) ────────────────────────────────────────
    for ds in db.query(DocumentationSession).filter(DocumentationSession.admission_id == admission_id).all():
        events.append({
            "type": "session_divider",
            "entry_id": ds.id,
            "timestamp": ds.signed_at.isoformat() if ds.signed_at else None,
            "written_by_name": _staff_name(ds.signed_by),
            "data": {"shift": ds.shift, "note": ds.note},
            "rendered_text": f"Documentation signed off by {_staff_name(ds.signed_by)}",
        })
...
@router.post("/admissions/{admission_id}/documentation-session")
def sign_documentation_session(
    admission_id: int,
    body: dict,
    db: Session = Depends(get_db),
    current: Staff = Depends(get_current_staff),
):
    """Sign & close a documentation block — inserts a divider in the chart timeline."""
    adm = db.query(Admission).filter(
        Admission.id == admission_id,
        Admission.clinic_id == current.clinic_id,
    ).first()
    ...
    h = new_hour = datetime.utcnow().hour
    shift = "morning" if 6 <= h < 14 else ("afternoon" if 14 <= h < 22 else "night")

    ds = DocumentationSession(
        clinic_id=current.clinic_id,
        admission_id=admission_id,
        signed_by=current.id,
        shift=body.get("shift", shift),
        note=body.get("note"),
    )
    db.add(ds)
    db.commit()
    db.refresh(ds)
    return {"id": ds.id, "signed_at": ds.signed_at.isoformat(), "message": "Documentation session signed off"}
...
# ── Assessment Templates (clinic-facing) ──────────────────────────────────────
...
def get_assessment_templates(
    ...
    """Return all active assessment templates visible to this clinic/department."""
    from app.models.models import AssessmentTemplate, TemplateAssignment
    clinic_id = current.clinic_id

    assigned_ids = set()
    for a in db.query(TemplateAssignment).all():
        if a.scope == "all":
    ...
    # Also include clinic-created templates (scope=clinic, clinic_id=mine)
    own = db.query(AssessmentTemplate).filter(
        AssessmentTemplate.scope == "clinic",
        AssessmentTemplate.clinic_id == clinic_id,
        AssessmentTemplate.is_active == True,
    ).all()
    own_ids = {t.id for t in own}

    all_ids = assigned_ids | own_ids
    if not all_ids:
        return []

    templates = db.query(AssessmentTemplate).filter(
        AssessmentTemplate.id.in_(all_ids),
        AssessmentTemplate.is_active == True,
    ).order_by(AssessmentTemplate.specialty, AssessmentTemplate.name).all()
