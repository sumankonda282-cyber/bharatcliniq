...
safe_cols = [
    ...
    # ── Phase 0: Org type & inpatient foundation ────────────────────────────────
    ...
    \"ALTER TABLE admissions ADD COLUMN IF NOT EXISTS care_team_ids INTEGER[]\",
    \"CREATE TABLE IF NOT EXISTS vital_signs (id SERIAL PRIMARY KEY, clinic_id INTEGER REFERENCES clinics(id) NOT NULL, admission_id INTEGER REFERENCES admissions(id) NOT NULL, patient_id INTEGER REFERENCES patients(id) NOT NULL, temperature NUMERIC(5,2), pulse INTEGER, respiratory_rate INTEGER, bp_systolic INTEGER, bp_diastolic INTEGER, spo2 INTEGER, weight NUMERIC(6,2), height NUMERIC(6,2), bmi NUMERIC(5,2), gcs INTEGER, pain_score INTEGER, blood_glucose NUMERIC(6,2), notes TEXT, recorded_by INTEGER REFERENCES staff(id), recorded_at TIMESTAMP DEFAULT NOW())\",
    \"CREATE TABLE IF NOT EXISTS nursing_notes (id SERIAL PRIMARY KEY, clinic_id INTEGER REFERENCES clinics(id) NOT NULL, admission_id INTEGER REFERENCES admissions(id) NOT NULL, patient_id INTEGER REFERENCES patients(id) NOT NULL, note_type VARCHAR(30) DEFAULT 'general', content TEXT NOT NULL, shift VARCHAR(20), is_signed BOOLEAN DEFAULT FALSE, signed_by INTEGER REFERENCES staff(id), signed_at TIMESTAMP, signer_credentials VARCHAR(100), created_by INTEGER REFERENCES staff(id), created_at TIMESTAMP DEFAULT NOW())\",
    \"CREATE TABLE IF NOT EXISTS medication_administrations (id SERIAL PRIMARY KEY, clinic_id INTEGER REFERENCES clinics(id) NOT NULL, admission_id INTEGER REFERENCES admissions(id) NOT NULL, patient_id INTEGER REFERENCES patients(id) NOT NULL, order_id INTEGER, drug_name VARCHAR(200) NOT NULL, dose VARCHAR(100), route VARCHAR(50), scheduled_at TIMESTAMP, administered_at TIMESTAMP, status VARCHAR(20) DEFAULT 'scheduled', reason_held TEXT, administered_by INTEGER REFERENCES staff(id), witness_by INTEGER REFERENCES staff(id), site VARCHAR(100), batch_number VARCHAR(50), notes TEXT, created_at TIMESTAMP DEFAULT NOW())\",
    \"CREATE TABLE IF NOT EXISTS ward_rounds (id SERIAL PRIMARY KEY, clinic_id INTEGER REFERENCES clinics(id) NOT NULL, admission_id INTEGER REFERENCES admissions(id) NOT NULL, patient_id INTEGER REFERENCES patients(id) NOT NULL, round_type VARCHAR(30) DEFAULT 'ward_round', subjective TEXT, objective TEXT, assessment TEXT, plan TEXT, is_signed BOOLEAN DEFAULT FALSE, signed_by INTEGER REFERENCES staff(id), signed_at TIMESTAMP, signer_credentials VARCHAR(100), created_by INTEGER REFERENCES staff(id), created_at TIMESTAMP DEFAULT NOW())\",
    \"CREATE TABLE IF NOT EXISTS progress_notes (id SERIAL PRIMARY KEY, clinic_id INTEGER REFERENCES clinics(id) NOT NULL, admission_id INTEGER REFERENCES admissions(id) NOT NULL, patient_id INTEGER REFERENCES patients(id) NOT NULL, note_type VARCHAR(30) DEFAULT 'progress', content TEXT NOT NULL, is_signed BOOLEAN DEFAULT FALSE, signed_by INTEGER REFERENCES staff(id), signed_at TIMESTAMP, signer_credentials VARCHAR(100), addendum TEXT, addendum_by INTEGER REFERENCES staff(id), addendum_at TIMESTAMP, created_by INTEGER REFERENCES staff(id), created_at TIMESTAMP DEFAULT NOW())\",
    \"CREATE TABLE IF NOT EXISTS discharge_summaries (id SERIAL PRIMARY KEY, clinic_id INTEGER REFERENCES clinics(id) NOT NULL, admission_id INTEGER REFERENCES admissions(id) UNIQUE NOT NULL, patient_id INTEGER REFERENCES patients(id) NOT NULL, presenting_complaint TEXT, history TEXT, examination TEXT, investigations TEXT, diagnosis TEXT, hospital_course TEXT, procedures_performed TEXT, discharge_condition VARCHAR(50), discharge_instructions TEXT, follow_up TEXT, medications_at_discharge TEXT, is_signed BOOLEAN DEFAULT FALSE, signed_by INTEGER REFERENCES staff(id), signed_at TIMESTAMP, signer_credentials VARCHAR(100), created_by INTEGER REFERENCES staff(id), created_at TIMESTAMP DEFAULT NOW())\",
    \"CREATE TABLE IF NOT EXISTS inpatient_charges (id SERIAL PRIMARY KEY, clinic_id INTEGER REFERENCES clinics(id) NOT NULL, admission_id INTEGER REFERENCES admissions(id) NOT NULL, patient_id INTEGER REFERENCES patients(id) NOT NULL, charge_type VARCHAR(30) NOT NULL, description VARCHAR(300) NOT NULL, quantity INTEGER DEFAULT 1, unit_price NUMERIC(10,2) NOT NULL, total NUMERIC(10,2) NOT NULL, charge_date DATE DEFAULT CURRENT_DATE, notes TEXT, created_by INTEGER REFERENCES staff(id), created_at TIMESTAMP DEFAULT NOW())\",
    \"CREATE TABLE IF NOT EXISTS inpatient_bills (id SERIAL PRIMARY KEY, clinic_id INTEGER REFERENCES clinics(id) NOT NULL, admission_id INTEGER REFERENCES admissions(id) UNIQUE NOT NULL, patient_id INTEGER REFERENCES patients(id) NOT NULL, bill_number VARCHAR(30) UNIQUE NOT NULL, subtotal NUMERIC(12,2) DEFAULT 0, discount NUMERIC(10,2) DEFAULT 0, tax NUMERIC(10,2) DEFAULT 0, total NUMERIC(12,2) DEFAULT 0, advance_paid NUMERIC(12,2) DEFAULT 0, insurance_amount NUMERIC(12,2) DEFAULT 0, balance_due NUMERIC(12,2) DEFAULT 0, status VARCHAR(20) DEFAULT 'draft', generated_at TIMESTAMP, finalized_at TIMESTAMP, paid_at TIMESTAMP, created_by INTEGER REFERENCES staff(id), created_at TIMESTAMP DEFAULT NOW())\",
    # ── CPOE: Computerised Provider Order Entry ─────────────────────────────────
    \"CREATE TABLE IF NOT EXISTS medication_orders (id SERIAL PRIMARY KEY, clinic_id INTEGER REFERENCES clinics(id) NOT NULL, admission_id INTEGER REFERENCES admissions(id) NOT NULL, patient_id INTEGER REFERENCES patients(id) NOT NULL, drug_name VARCHAR(200) NOT NULL, generic_name VARCHAR(200), dose VARCHAR(100) NOT NULL, route VARCHAR(50) NOT NULL, frequency VARCHAR(30) NOT NULL, duration_days INTEGER, start_date DATE, end_date DATE, instructions TEXT, is_prn BOOLEAN DEFAULT FALSE, prn_reason VARCHAR(200), is_stat BOOLEAN DEFAULT FALSE, is_continuous BOOLEAN DEFAULT FALSE, iv_rate VARCHAR(50), iv_fluid VARCHAR(100), iv_volume_ml INTEGER, status VARCHAR(20) DEFAULT 'active', ordered_by INTEGER REFERENCES staff(id) NOT NULL, ordered_at TIMESTAMP DEFAULT NOW(), discontinued_by INTEGER REFERENCES staff(id), discontinued_at TIMESTAMP, discontinue_reason TEXT, notes TEXT)\",
    \"CREATE TABLE IF NOT EXISTS clinical_orders (id SERIAL PRIMARY KEY, clinic_id INTEGER REFERENCES clinics(id) NOT NULL, admission_id INTEGER REFERENCES admissions(id) NOT NULL, patient_id INTEGER REFERENCES patients(id) NOT NULL, order_type VARCHAR(30) NOT NULL, order_detail VARCHAR(300) NOT NULL, priority VARCHAR(20) DEFAULT 'routine', instructions TEXT, status VARCHAR(20) DEFAULT 'pending', ordered_by INTEGER REFERENCES staff(id) NOT NULL, ordered_at TIMESTAMP DEFAULT NOW(), acknowledged_by INTEGER REFERENCES staff(id), acknowledged_at TIMESTAMP, completed_by INTEGER REFERENCES staff(id), completed_at TIMESTAMP, result_notes TEXT, notes TEXT)\",
    \"ALTER TABLE medication_administrations ADD COLUMN IF NOT EXISTS order_id INTEGER REFERENCES medication_orders(id)\",
]
try:
