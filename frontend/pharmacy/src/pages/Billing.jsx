...
const RETURN_REASONS = ['Damaged', 'Wrong Medicine', 'Patient Refused', 'Expired', 'Other']
...
function SalesReturns() {
  ...
  const loadReturns = useCallback(() => {
    setLoading(true)
    api.get('/pharmacy/returns')
      .then(r => setReturns(Array.isArray(r) ? r : []))
      .catch(() => setReturns([]))
  ...
  async function searchInvoice() {
    ...
    try {
      ...
      const all = await api.get('/billing/invoices', { params: { limit: 200 } })
      const match = (Array.isArray(all) ? all : []).find(
        inv => inv.invoice_number === invoiceSearch.trim() ||
      ...
      if (match) {
        const detail = await api.get(`/billing/invoices/${match.id}`)
        setFoundInvoice(detail)
        setSelectedItems((detail.items || []).map(i => ({
          ...i, return_qty: i.quantity, selected: false,
        })))
      } else {
  ...
  function toggleItem(idx) {
    setSelectedItems(prev => prev.map((i, n) => n === idx ? { ...i, selected: !i.selected } : i))
  }

  function updateQty(idx, qty) {
    setSelectedItems(prev => prev.map((i, n) => n === idx ? { ...i, return_qty: qty } : i))
  }

  const returnItems = selectedItems.filter(i => i.selected)
  const total = returnItems.reduce((s, i) => s + Number(i.return_qty) * Number(i.unit_price || 0), 0)

  async function submitReturn() {
    if (returnItems.length === 0) { setErr('Select at least one item to return'); return }
    setSubmitting(true)
    setErr('')
    try {
      ...
      setErr(ex.message || 'Return failed')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-gray-500">{returns.length} credit note(s)</p>
        <button onClick={() => { setShowCreate(true); setFoundInvoice(null); setSelectedItems([]); setInvoiceSearch(''); setErr('') }} className="btn-primary text-sm py-1.5">
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-[#0F2557]">Sales Return / Credit Note</h3>
            ...
            <div className="flex gap-2 mb-4">
              <input
                ...
                placeholder="Invoice number or patient/customer name"
                value={invoiceSearch}
                onChange={e => setInvoiceSearch(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && searchInvoice()}
              />
              <button onClick={searchInvoice} disabled={searching} className="btn-secondary px-4">
            ...
            {foundInvoice && (
              <div className="space-y-4">
                <div className="bg-gray-50 rounded-xl p-3 text-sm">
                  <p><strong>Invoice:</strong> {foundInvoice.invoice_number}</p>
                  <p><strong>Customer:</strong> {foundInvoice.customer_name || foundInvoice.patient_name || 'Walk-in'}</p>
                  <p><strong>Date:</strong> {foundInvoice.created_at ? new Date(foundInvoice.created_at).toLocaleDateString('en-IN') : '—'}</p>
                  <p><strong>Total:</strong> ₹{Number(foundInvoice.total || 0).toFixed(2)}</p>
                </div>

                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Select Items to Return</p>
                <div className="space-y-2">
                  {selectedItems.map((item, idx) => (
                    <div key={idx} className={`border rounded-xl p-3 transition-colors ${item.selected ? 'border-[#0F2557] bg-blue-50' : 'border-gray-200'}`}>
                      <div className="flex items-center gap-3">
                        <input type="checkbox" checked={item.selected} onChange={() => toggleItem(idx)} className="w-4 h-4" />
                        <div className="flex-1">
                          <div className="text-sm font-medium">{item.description || item.medicine_name || '—'}</div>
                          <div className="text-xs text-gray-400">Billed: {item.quantity} × ₹{Number(item.unit_price || 0).toFixed(2)}</div>
                        </div>
                        {item.selected && (
                          <div className="flex items-center gap-2">
                            <label className="label text-xs">Return Qty</label>
                            <input
                              type="number" className="input py-1 w-20 text-sm"
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select className="input" value={refundMethod} onChange={e => setRefundMethod(e.target.value)}>
                      {['cash', 'card', 'upi', 'credit_note'].map(m => (
                ...
                {returnItems.length > 0 && (
                  <div className="bg-green-50 border border-green-200 rounded-xl p-3 text-sm">
                    <strong>Refund Amount: ₹{total.toFixed(2)}</strong>
                    <span className="text-gray-500 ml-2">({returnItems.length} item(s))</span>
                  </div>
                )}
              </div>
            )}
              ...
              <tbody className="divide-y divide-gray-100">
                {returns.map(r => (
                  <tr key={r.id} className="tr-hover">
                    <td className="td font-mono text-sm text-[#0F2557] font-semibold">{r.return_number}</td>
                    <td className="td text-gray-500 text-xs">{r.invoice_number || '—'}</td>
                    <td className="td">{r.reason}</td>
                    <td className="td font-semibold text-green-700">₹{Number(r.total_refund || 0).toFixed(2)}</td>
                    <td className="td capitalize text-gray-500">{r.refund_method || '—'}</td>
