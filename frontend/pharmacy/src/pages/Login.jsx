...
import { Eye, EyeOff, AlertCircle, Pill, Package, CheckCircle } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen flex">
      <div className="hidden lg:flex lg:w-1/2 flex-col justify-between p-10 text-white" style={{ background: 'linear-gradient(145deg,#0F2557 0%,#1a3a7a 100%)' }}>
        <div><div className="text-2xl font-extrabold">BHaratCliniq</div><div className="text-xs font-semibold mt-1 tracking-wider uppercase" style={{ color: '#F5821E' }}>Pharmacy Portal</div></div>
        <div>
          <h2 className="text-4xl font-extrabold mb-4">Dispense Smarter,<br /><span style={{ color: '#F5821E' }}>Serve Faster.</span></h2>
          <p className="text-blue-200 mb-8">View prescriptions, manage stock, and dispense medicines with a click.</p>
          {[{icon:Pill,text:'View & dispense pending prescriptions'},{icon:Package,text:'Manage medicine inventory & stock'},{icon:CheckCircle,text:'Track dispensing history'}].map(({icon:Icon,text})=>(
        ...
        <div className="text-xs" style={{ color: '#93c5fd' }}>BHaratCliniq · Pharmacy Portal</div>
      </div>
      <div className="flex-1 flex items-center justify-center p-6 bg-gray-50">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-md border border-gray-100 p-8">
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <button type="submit" disabled={loading} className="w-full btn-primary justify-center py-3">{loading?<><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/>Signing in…</>:'Sign In to Pharmacy'}</button>
          </form>
        </div>
      </div>
    </div>
