...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (token) { api.get('/auth/staff/me').then(u => setUser(u)).catch(() => localStorage.clear()).finally(() => setLoading(false)) }
    else setLoading(false)
  }, [])
  const login = async (identifier, password) => {
    const r = await api.post('/auth/staff/login', { identifier, password })
    if (!['lab_technician','clinic_admin'].includes(r.role)) throw new Error('Access denied. This portal is for lab staff only.')
